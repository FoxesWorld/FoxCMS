<?php 
	...
	$init = new init(array("preInit" => true, "init" => true, "postInit" => false), false);
...
<div class="login-container">
  <section class="login" id="login">
    <header>
      <h2><? echo $config["title"]?></h2>
      <h4>Login</h4>
    </header>
    <form class="login-form" action="/" method="post">
      <input type="text" name="login" class="login-input" placeholder="User" required autofocus/>
      <input type="password" class="login-input" name="password" placeholder="Password" required/>
	  <input type="hidden" name="userAction" value="auth" />
	  <input type="hidden" name="key" value="<?echo $config["javascript"]["secureKey"]?>" />
