<?php
	...
	class PluginsScanner extends smartyInit {
		...
		function pluginsInclude(){
			...
			foreach($scanDir as $object) {
				...
				if(is_dir($currentObjectDir)){
					if($this->pluginConfig($currentObjectDir)["access"]) {
		...
		private function pluginConfig($DIR) {
			$configArray = array();
			$configArray["access"] = true;
			...
			if(file_exists($pluginOptFile)){
				...
				foreach($pluginOptions as $key => $value){
					switch($key){
						case "pluginGroup":
							$configArray["access"] = CheckUserAccess::checkAccess(init::$usrArray['user_group'], $value);				
						break;
