	...
	if($nick && $hash) {
		if(strcmp(md5(md5($nick.$secret."mcrate")), $hash) == 0) {
			...
			if($select['name']) {
				...
				$much = $db->query("SELECT * FROM vote_table WHERE uuid='{$select['uuid']}' AND `from`='mcrate' AND date_text LIKE '%.".date("m.Y")."%'");
				...
				$check = $db->query("SELECT * FROM vote_table WHERE uuid='{$select['uuid']}' AND `from`='mcrate' AND date_text='".date("d.m.Y")."'");
				if(!$db->num_rows($check)) {
					$db->query("INSERT INTO `vote_table` (`id`, `uuid`, `timestamp`, `date_text`, `from`) VALUES (NULL, '{$select['uuid']}', '".time()."', '".date("d.m.Y")."', 'mcrate')");
