...
class UserOptions extends init {
    ...
    function __construct($db, $logger) {
        ...
        $GetOption = new GetOption(init::$usrArray['login'], $db, $logger);
    }
        ...
        foreach (self::$allOptionsArray["optionObjects"] as $optionObject) {
			if(!is_dir($scanDir.DIRECTORY_SEPARATOR.$optionObject)) {
				if(strpos($optionObject, ".ftpl")) {
					...
					self::$allOptionsArray["options"][$optionName]["optContent"] = $optionContents;
					self::$allOptionsArray["options"][$optionName]["optPath"] = $filePath;	
    ...
    private function userOptionsArrayFilling() {
        ...
        foreach (self::$allOptionsArray["options"] as $optionFname => $optionConf) {
            if ($this->isOption($optionFname)) {
                ...
                $canAccess = CheckUserAccess::checkAccess(init::$usrArray["user_group"], $decodedOption["optionGroup"]);
                if ($canAccess === true) {
					init::$userOptions[$optionFname]["fullPath"] = self::$allOptionsArray["options"][$optionFname]["optPath"];
                    switch ($decodedOption["type"]) {
                        case "plainText":
                            case "page":
                                init::$userOptions[$optionFname]["optContent"] = $this->getContentByName($optionFname);
                                init::$userOptions[$optionFname]["optSettings"] = $this->getConfigByName($optionFname);
                                init::$userOptions[$optionFname]["optTitle"] = $decodedOption["optionTitle"];
                                init::$userOptions[$optionFname]["optPreText"] = $decodedOption["optionPreText"];
                                break;
                    ...
                    init::$userOptions[$optionFname]["optType"] = $decodedOption["type"];
                    $optionsAmount++;
                }
            }
        }
        init::$userOptions["optionsAmount"] = $optionsAmount;
    }


    private function userMenuBuild() {
        if (isset(init::$userOptions["optionsAmount"])) {
            $count = 0;
            foreach (init::$userOptions as $key => $value) {
                if ($this->isOption($key)) {
                    $thisConfig = json_decode(init::$userOptions[$key]["optSettings"], true);
                    ...
                    if ($count === init::$userOptions["optionsAmount"] -1) {
                        if (init::$usrArray['user_group'] !== 5) {
                            self::$builtMenu["optionArray"][] = $this->addTheLast;
                            $count++;
                        }
                    }
                    init::$userOptions["optionNames"][] = $key;
                    $count++;
					unset($arr);
                }
            }
