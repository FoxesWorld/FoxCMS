...
class GetOption extends UserOptions {
	...
	public static function getPageContent($pageObject, $filePath){
		 if (in_array($pageObject, init::$userOptions["optionNames"])) {
                $optionSettings = json_decode(UserOptions::getOptionData($pageObject), true);
                $optionBody = init::$userOptions[$pageObject];
                switch ($optionSettings["type"]) {
                    ...
                    case "pageContent":
                        $pageTemplate = self::getPageContents($optionBody["optContent"]);
                        break;
                }
                return preg_replace('|(<useroption>).*(</useroption>)|Uis', '', $pageTemplate);
