...
/*FoxesModule%>
{
	"version": "V 0.1.8 Beta",
	"description": "Module to select options for current user",
    ...
    private function userOptionsArrayFilling() {
        ...
        foreach (self::$allOptionsArray["options"] as $optionFname => $optionConf) {
            if ($this->isOption($optionFname)) {
                ...
                if ($canAccess === true) {
					self::$userOptions[$optionFname]["fullPath"] = self::$allOptionsArray["options"][$optionFname]["optPath"];
                        ...
                        if ($this->isOptionFillValue($optArrayElement)) {
                            $arr[$key][$optArrayElement] = $value;
							 if(isset($arr[$key]['type'])) {
								 $adlVarsForcOpt = $this->additionalVars[$arr[$key]['type']];
								switch($arr[$key]['type']){
									case "page":
										foreach($adlVarsForcOpt as $additionalVar){
											if(isset($thisConfig[$additionalVar])) {
												$arr[$key][$additionalVar] = $thisConfig[$additionalVar];
											}
										}
