					...
					if(isset($moduleMainFilePath)) {
						...
						$moduleInfo = $this->readModuleInfo($moduleMainFilePath);
							...
							if($this->thisGroup !== null) {
								if(is_array($this->thisGroup)){
									$groupArr = array();
									foreach($this->thisGroup as $val){
										$mdlGroup = new groupAssociacion($val, $this->db);
										$groupArr[] = $mdlGroup->userGroupName();
									}
									$moduleGroups = implode(',', $groupArr);
								} else {
									$mdlGroup = new groupAssociacion($this->thisGroup, $this->db);
									$moduleGroups = $mdlGroup->userGroupName();
								}
							} else {
								$moduleGroups = "*";
							}
