...
export function FoxesRequest(path, args, timeDelay) {
    ...
    this.sendPost = function (params, after, antitimeDelay) {
        ...
        if (this.timeDelay != false && antitimeDelay == true /*&& this.timeDelay_count ++ == this.timeDelay_maxCount*/) {
            ...
            if (time_ < this.timeDelay) {
                return after({
                    status: "error",
                    message: "Please wait for " + ((this.timeDelay - time_) / 1000 + 1).toFixed(0) + " seconds..."
                });
