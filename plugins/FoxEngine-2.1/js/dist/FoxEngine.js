...
var FoxesInputHandler = class {
  ...
  collectFormData(form2) {
    ...
    inputFields.forEach((input) => {
      ...
      if (!name) return;
      const inputsInForm = $(form2).find(`[name="${CSS.escape(name)}"]`);
...
// modules/User/BadgeManager.js
var BadgeManager = class {
  constructor(foxEngine2, apiService) {
    this.foxEngine = foxEngine2;
    this.apiService = apiService;
    this.lastUser = null;
    this.loadedBadgeKeys = /* @__PURE__ */ new Set();
    this.cachedBadges = [];
  }
  getBadgeKey(badge) {
    return badge.id ? String(badge.id) : `${badge.badgeName}_${Math.floor(new Date(badge.acquiredDate).getTime() / 1e3)}`;
  }
  reset() {
  ...
  async getBadgesArray(user) {
    ...
    try {
      ...
      const result = badges.map((badge) => ({
        badgeId: badge.id || `${badge.badgeName}_${Math.floor(new Date(badge.acquiredDate).getTime() / 1e3)}`,
        badgeName: badge.badgeName,
...
// modules/User/PlaytimeWidgetGenerator.js
var PlaytimeWidgetGenerator = class {
  ...
  async createPlayTimeWidget(raw) {
    try {
      ...
      const html = await this.generate(data2);
  ...
  async generate(data2) {
    const servers = this.parseServersData(data2);
    if (!servers.length) {
  ...
  parseServersData(serversRaw) {
    if (Array.isArray(serversRaw)) {
      return serversRaw.filter((s) => s.serverName && !isNaN(Number(s.totalTime))).map((s) => ({
        name: s.serverName,
...
// modules/User/OptionsManager.js
var OptionsManager = class _OptionsManager {
  constructor(foxEngine2, apiService) {
    this.foxEngine = foxEngine2;
    this.apiService = apiService;
    this.optionAmount = 0;
  ...
  renderOption(name, opt) {
    const tpl = _OptionsManager.templates[opt.type];
  ...
  static templates = {
    ...
    userOption: ({ optionClass, optionPreText, optionTitle, func: func2 }) => `
            <li class="${optionClass}">
...
var User = class {
  constructor(foxEngine2) {
    this.foxEngine = foxEngine2;
    this.api = new ApiService(foxEngine2);
    this.badgeManager = new BadgeManager(foxEngine2, this.api);
    this.skinManager = new SkinManager(foxEngine2, this.api);
    this.playtimeWidgetManager = new PlaytimeWidgetGenerator(foxEngine2);
    this.optionsManager = new OptionsManager(foxEngine2, this.api);
    this.uiRenderer = new UIRenderer(foxEngine2);
  ...
  async getUserData(login) {
    ...
    try {
      const data2 = await this.api.request({ user_doaction: "getUserData", login });
  ...
  async createPlayTimeWidget(raw) {
    try {
      ...
      const html = await playTimeWidget.generate(data2);
      const container = document.querySelector(".playtime-widget-container");
...
var InfoBox = class {
  constructor(foxEngine2) {
    this.foxEngine = foxEngine2;
    this.cookieManager = foxEngine2.cookieManager;
  ...
  async setBox(container) {
    ...
    let data2 = await this._getInfoData();
    data2 = data2[0];
    ...
    let html = await this.foxEngine.replaceTextInTemplate(template, {
      ...
      text: data2.text,
      image: data2.image,
      button_text: data2.button_text,
      button_url: data2.button_url
    });
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = html;
    if (!data2.button_text || data2.button_text.trim() === "") {
